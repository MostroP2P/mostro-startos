...
export const inputSpec = InputSpec.of({
  ...
  db_password: Value.text({
    ...
    description:
      'Optional database password for Mostro SQLite database. Leave empty for no password protection.',
...
export const databaseSettings = sdk.Action.withInput(
  ...
  async ({ effects }) => ({
    ...
    warning:
      'If you set a password, you will need to provide it every time the service restarts.',
    allowedStatuses: 'any',
    group: 'Mostro',
    visibility: 'enabled',
  }),
  ...
  async ({ effects, input }) => {
    ...
    if (passwordRequired) {
      console.log(
        'Database password protection enabled. Password will be required at service restart.',
      )
    } else {
      console.log(
        'Database password protection disabled. Service will start without password.',
      )
    }
  },
)
