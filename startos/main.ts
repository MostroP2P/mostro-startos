...
export const main = sdk.setupMain(
  async ({ effects, started }: { effects: any; started: any }) => {
    /**
     * ======================== Setup (optional) ========================
     *
    ...
    let mostroCommand: [string, ...string[]] = [
      'mostrod',
      '-d',
      '/mostro',
      '-c',
    ...
    if (dbPasswordRequired && dbPassword) {
      ...
      console.log(
        'Database password protection enabled. Starting with password.',
      )
      ...
      console.log(
        'Database password protection disabled. Starting without password.',
      )
    }
        ...
        ready: {
          ...
          fn: async (): Promise<{
            ...
            try {
              ...
              const tomlConfig = await daemon_settings
                .read((s) => s)
                .const(effects)
              ...
              if (result.exitCode !== 0) {
                console.log(
                  'Trying with absolute proto path and import path...',
                )
              ...
              if (result.exitCode !== 0) {
                console.log(
                  `gRPC call failed with exit code ${result.exitCode}`,
                )
